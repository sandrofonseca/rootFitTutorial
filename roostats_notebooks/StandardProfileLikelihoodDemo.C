...
double confLevel = 0.683; // confidence level
double nScanPoints = 30;   // number of points to scan 
...
void StandardProfileLikelihoodDemo(const char* infile = "",
  ...
  pl.SetConfidenceLevel(confLevel); // CL% interval
  LikelihoodInterval* interval = pl.GetInterval();

  // print out the iterval on the first Parameter of Interest
  RooRealVar* firstPOI = (RooRealVar*) mc->GetParametersOfInterest()->first();
  cout << "\n" <<  confLevel << "% interval on " <<firstPOI->GetName()<<" is : ["<<
