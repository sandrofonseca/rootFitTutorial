...
void StandardBayesianMCMCDemo(const char* infile = "",
  ...
  */
  ...
  mcmc.SetConfidenceLevel(confLevel); // CL  interval
  //  mcmc.SetProposalFunction(*pf);
  mcmc.SetProposalFunction(sp);
  mcmc.SetNumIters(numIterations);         // Metropolis-Hastings algorithm iterations
  ...
  cout << "\n" << 100*confLevel << "% interval on " <<firstPOI->GetName()<<" is : ["<<
